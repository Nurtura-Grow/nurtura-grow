...
<div class="mobile-menu md:hidden">
    ...
    <ul class="border-t border-theme-24 py-5 hidden">
        @foreach ($side_menu as $menu)
            ...
            @else
                <li>
                    <a href="{{ isset($menu['layout']) ? route('page', [ 'pageName' => $menu['page_name']]) : 'javascript:;' }}" class="{{ $first_page_name == $menu['page_name'] ? 'menu menu--active' : 'menu' }}">
                    ...
                    @if (isset($menu['sub_menu']))
                        <ul class="{{ $first_page_name == $menu['page_name'] ? 'menu__sub-open' : '' }}">
                            @foreach ($menu['sub_menu'] as $subMenu)
                                <li>
                                    <a href="{{ isset($subMenu['layout']) ? route('page', [ 'pageName' => $subMenu['page_name']]) : 'javascript:;' }}" class="{{ $second_page_name == $subMenu['page_name'] ? 'menu menu--active' : 'menu' }}">
                                        <div class="menu__icon">
